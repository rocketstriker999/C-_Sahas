...
requestHelper.requestServer({
    ...
    if (jsonResponse.isTaskSuccess == 'true') {
        ...
        data.forEach(img => images.push(`${requestHelper.serverAddress}/thumbnails/${img.carousel_img}`))
        showCarousel();
    }
    else
...
function showCarousel() {
    currentImageIndex = currentImageIndex + 1 < images.length ? currentImageIndex + 1 : 0;
    slideContainer.innerHTML = `<div class="mySlides"><img src="${images[currentImageIndex]}" style="width:100%"></div>`;
    setTimeout(() => {
        showCarousel();
    }, 2000)
}
